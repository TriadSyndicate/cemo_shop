...
import { useAppConfig } from "@/components/composables/appConfig.js";
import { useDeskree } from "@/components/composables/useDeskTree.js";
...
const loggedInUser = computed(() => useDeskree.loggedInUser.value);
...
<template>
  <nav
    ...
    "
  >
    <div class="flex flex-wrap w-full md:w-auto justify-between px-0 mx-0">
      <div class="dark:text-white w-full md:w-auto">
        <!--Logo-->
        <div class="flex justify-between">
          <a href="#">
            <svg width="150" height="32.125" viewBox="0 0 150 32.125">
              <g
                id="Group_330"
                data-name="Group 330"
            ...
            <h2 class="font-bold text-2xl">{{ siteName }}</h2>
          </a>
          <Bars4Icon
            @click="toggleMenu"
            class="h-6 w-6 cursor-pointer md:hidden dark:text-white"
          />
        </div>
      </div>
      <div
      ...
      >
        ...
        >
          ...
          >
            <a
              ...
              class="
                ...
                hover-underline-animation-medium
              "
              >{{ category }}</a
            >
          </li>
          <li class="flex mt-3 group">
            <ShoppingBagIcon class="h-6 w-6 dark:text-white dark:group-hover:text-purple-500 mr-1 cursor-pointer" />
            <p class="dark:text-white text-base dark:group-hover:text-purple-500">
              {{
                cart.itemCount
              }}
            </p>
          ...
          <li class="flex mt-3 group">
            <UserCircleIcon class="h-6 w-6 dark:text-white dark:group-hover:text-purple-500 mr-1" />
            <a class="dark:text-white flex italic dark:group-hover:text-purple-500" href=""
              >Log In</a
            >
          </li>
        </ul>
...
.hover-underline-animation-medium::after {
  content: '';
  position: absolute;
  width: 100%;
  height: 2px;
...
.hover-underline-animation-medium:hover::after {
  transform: scaleX(1);
}
</style>